...
- ✅ **v4.2 (4 Jun) — klarifikasi anggaran** `workshop/Proposal-LEAP-2036-v4.2.md`: fix mismatch total (tabel komparasi sempat ketik Rp 500k → dibetulin Rp 480k) + perjelas label 3 item RAB (laminasi/poster pos/papan keragaman) biar reviewer ga bingung. **Prosa naratif NOL diubah** (cuma angka + label item). `panduan-tim/Anggaran-Rinci` disinkron (13→11 item: name tag + sertifikat cetak dibuang, total 480k).
- ✅ **RAB-trim RESOLVED: TIDAK dipangkas — kunci Rp 480.000.** Bedah ulang per item: ketiganya kepake & lebih mahal kalau dipotong (laminasi = reusable lintas 2 SMA, hemat vs reprint · poster pos = backup tanpa proyektor, 8 sudut Tujuan ga bisa diproyektor · papan keragaman = visual fisik no-juara). Udah < pagu 500k + ada buffer 30k.
- ✅ **v4.3 (4 Jun) — audit dingin → PURE 2 SMA** `workshop/Proposal-LEAP-2036-v4.3.md` (**= CANONICAL, supersede v4.2**). Audit nemu 8 temuan; akar = dual-scenario (1 SMA + 2 SMA) bikin rundown & RAB inkonsisten. Keputusan Dex: **collapse pure 2 SMA.** Dibereskan: Skema LPPM-1 (1 SMA) dibuang · tabel komparasi → spesifikasi tunggal · rundown di-reshape "1 siklus utuh per sekolah, dipadatkan" (durasi `<3 jam` → `~1–1,5 jam`, kill overclaim 15 jam) · "Fun Day" sbg modul → "Penutup & Apresiasi" (Fun Day = event terpisah) · "DISC" mentah → "kuis kecenderungan (dinilai sendiri)" · indikator "4 dari 5 hari" → "mayoritas hari di sekolahnya" · penutup "skenario manapun" → "2 SMA". Prosa method/filosofi (BAB I/II/III-inti) NOL disentuh. Sisa LOW belum disentuh (opsional): media artikel "untuk disubmit" + jargon padat BAB III.
- ✅ **v4.4 (4 Jun) = FINAL/CANONICAL** `workshop/Proposal-LEAP-2036-v4.4.md` (versi disimpen yang BENER). Dex turun tangan revisi (hand-merge dari Docs): **apresiasi naik → 6 amplop×Rp10k = Rp60k, TOTAL Rp 493.000** (masih < 500k) · profil → English (Balanced Builder/Burnout Achiever/Late Bloomer) · §5 jadi "Filosofi Parallel Journeys" · target-terukur table dihapus. **Audit-ku fix 11 hal:** 5 error (typo "DISCa"→DISC · rundown "2023"→2033 · 2 goal nempel 1 baris→dipisah · total Rp493k nyangkut kolom Qty→Subtotal · penutup "skenario manapun LPPM-1/2" kontradiksi→"2 SMA") + 4 konsistensi (verifikasi "Formulir Kepribadian DISC"→kuis self-report · tabel "Antar Skenario"→"Spesifikasi Pelaksanaan" · media 5-10 materi/2-3 video→best-effort · item 1 "(kode di Kartu Skor)" + bullet name-tag→apresiasi direstore) + 2 typo sepele.
- ⏳ **NEXT:** Proposal **UDAH DIKUMPULIN** (versi Google Doc Dex — mungkin masih ada typo pre-fix; Dex: "yaudahlah"). Kalau sempat resubmit/revisi Doc → apply 6 fix 🔴 dari v4.4.md (before→after ada di chat). Lanjut: tunggu feedback APL/DPL · Syendhi uji sistem data · Eyecare training 13–19 Juni.
